...
# read font and set fonesize
font = ImageFont.truetype("readonly/fanwood-webfont.ttf", 55)

# create differnt channel intensitived images
intensitv = [0.1, 0.5, 0.9]
r, g, b = image.split()

# Notice that these images are stored in im list corresponding in contact sheet from top to button, then left to right
im = []
for i in intensitv:
    im.append(Image.merge("RGB", (r.point(lambda x: x*i), g, b)))
    im.append(Image.merge("RGB", (r, g.point(lambda x: x*i), b)))
    im.append(Image.merge("RGB", (r, g, b.point(lambda x: x*i))))

# create a contact sheet from different brightnesses
contact_sheet=PIL.Image.new(image.mode, (image.width*3,(image.height+75)*3))
x=0
y=0

# use to draw text on contact_sheet
draw_text = ImageDraw.Draw(contact_sheet)

#keep track of current channel and intensitv
chan = 0
itsv_idx = 0
for img in im:
    # Lets paste the current image into the contact sheet
    contact_sheet.paste(img, (x, y) )
    # draw text into contact sheet
    draw_text.text((x, y+img.height+20), "channal {} intensitv {}".format(chan, intensitv[itsv_idx]), font=font)
    # Now we update our Y position. If it is going to be the height of the image, then we set it to 0
    # and update X as well to point to the next "row" of the contact sheet.
    if y+first_image.height+75 == contact_sheet.height:
        y=0
        chan = 0
